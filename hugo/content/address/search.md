        ...
        .then(data => {
           ...
           if (data.addresses.length > 0) {
                ...
                if (currentMarker) {
                    ...
                    currentMarker.setLatLng({lat: data.addresses[0].lat, lon: data.addresses[0].lon});
                } else {
                    // Создание нового маркера
                    currentMarker = L.marker({lat: data.addresses[0].lat, lon: data.addresses[0].lon}).addTo(mymap);
                }
           }
        })
        .catch(error => {
...
var table = new Tabulator("#result", {
    ...
    selectableCheck:function(row){
        ...
        if (data.lat != "" && data.lon != "") {
            ...
            if (currentMarker) {
                // Перемещение существующего маркера
                currentMarker.setLatLng({lat: data.lat, lon: data.lon});
            } else {
                // Создание нового маркера
                currentMarker = L.marker({lat: data.lat, lon: data.lon}).addTo(mymap);
            }
        }
        table.deselectRow();
